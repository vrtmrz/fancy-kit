...
async function waitUntilClosed(testSession: ShowcaseTestSession): Promise<void> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await withObsidianPage(obsidianRemoteDebuggingPort(), async (page) => {
      await page.getByRole("heading", { name: "Obsidian Plugin Kit" }).waitFor({ timeout: 10_000 });
    });
    console.log(`Showcase opened in temporary vault: ${testSession.vault.path}`);
