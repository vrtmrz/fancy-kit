...
export type SafePathComponent = string & { readonly [safePathComponentBrand]: true };
...
function getUnsafeComponentReason(component: string): UnsafePathReason | undefined {
    if (component.length === 0) return "empty-component";
    if (component === "." || component === "..") return "dot-component";
    if (containsControlCharacter(component)) return "control-character";
    if (INVALID_PORTABLE_COMPONENT_CHARACTER.test(component)) return "invalid-character";
    if (/[. ]$/u.test(component)) return "trailing-dot-or-space";
    if (RESERVED_PORTABLE_COMPONENT.test(component)) return "reserved-name";
    return undefined;
}
 ...
 */
export function parseSafePathComponent(component: string): SafePathComponent {
    const reason = getUnsafeComponentReason(component);
    if (reason !== undefined) throw new UnsafePathError(component, reason, component);
    return component as SafePathComponent;
}
 ...
 */
export function isSafePathComponent(component: string): component is SafePathComponent {
 ...
 */
export function parseSafeRelativePath(path: string, options: SafeRelativePathOptions = {}): SafeRelativePath {
    if (path.length === 0) {
        if (options.allowEmpty === true) return path as SafeRelativePath;
    ...
    for (const component of path.split("/")) {
        const reason = getUnsafeComponentReason(component);
        if (reason !== undefined) throw new UnsafePathError(path, reason, component);
    }
 ...
 */
export function isSafeRelativePath(path: string, options: SafeRelativePathOptions = {}): path is SafeRelativePath {
    if (path.length === 0) return options.allowEmpty === true;
    if (path.startsWith("/") || path.startsWith("\\") || DRIVE_PREFIX.test(path)) return false;
...
export function resolvePathWithinRoot(
    ...
    if (safePath.length === 0) return root;
    ...
    if (root.length > 0) return `${root}${joinedRelativePath}`;
