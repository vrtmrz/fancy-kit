...
function base64ToArrayBufferNative(base64) {
    ...
    if (base64.length === 0)
        return new ArrayBuffer(0); // fix for empty array input or empty string input
    try {
 ...
 * @returns The converted ArrayBuffer.
 */
const base64ToArrayBuffer = isProposalArrayBufferBase64Available
 ...
 * @returns A Promise that resolves to the base64 string representation of the ArrayBuffer.
 */
const arrayBufferToBase64Single = isProposalArrayBufferBase64Available
 ...
 */
function arrayBufferToBase64Native(buffer) {
    const buf = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
    const result = buf.toBase64();
 ...
 * @returns A Promise that resolves to an array of base64 strings.
 */
async function arrayBufferToBase64Browser(buffer) {
    const buf = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
    if (buf.byteLength < QUANTUM)
        return [btoa(String.fromCharCode.apply(null, [...buf]))];
    ...
    do {
        ...
        const pBuf = new DataView(buf.buffer, buf.byteOffset + offset, Math.min(encodeChunkSize, buf.byteLength - offset));
 ...
 * @returns A Promise that resolves to an array of base64 strings.
 */
const arrayBufferToBase64 = isProposalArrayBufferBase64Available
...
function base64ToStringBrowser(base64) {
    try {
        if (typeof base64 != "string")
            return base64.map((e) => base64ToStringBrowser(e)).join("");
...
function base64ToStringNative(base64) {
    try {
        if (typeof base64 != "string")
            return base64.map((e) => base64ToStringNative(e)).join("");
 ...
 */
const tryConvertBase64ToArrayBuffer = isProposalArrayBufferBase64Available
 ...
 * @returns The converted ArrayBuffer if successful, otherwise false.
 */
function tryConvertBase64ToArrayBufferBrowser(base64) {
    try {
 ...
 * @returns The converted ArrayBuffer if successful, otherwise false.
 */
function tryConvertBase64ToArrayBufferNative(base64) {
    try {
        const b64F = base64.replace(/\r|\n/g, "");
        if (!regexpBase64.test(b64F)) {
            return false;
        }
        const buf = Uint8Array.fromBase64(b64F);
        if (buf.toBase64() !== b64F) {
            return false;
        }
...
export { arrayBufferToBase64, arrayBufferToBase64Browser, arrayBufferToBase64Native, arrayBufferToBase64Single, arrayBufferToBase64SingleBrowser, arrayBufferToBase64SingleNative, base64ToArrayBuffer, base64ToArrayBufferBrowser, base64ToArrayBufferInternalBrowser, base64ToArrayBufferNative, base64ToString, base64ToStringBrowser, base64ToStringNative, readString, tryConvertBase64ToArrayBuffer, tryConvertBase64ToArrayBufferBrowser, tryConvertBase64ToArrayBufferNative, writeString };
