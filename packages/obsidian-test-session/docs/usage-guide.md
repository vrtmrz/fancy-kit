...
4. pre-seed Vault trust state and ask `obsidian-cli` to open the Vault;
...
- `startObsidianPluginSession` accepts the prepared Vault, executables, artefact root, and environment overrides; and
...
| Exact active-Vault confirmation and UI-idle handling | [`src/ui.test.ts`](../src/ui.test.ts) | Real-Obsidian harness scripts under [`test/e2e-obsidian/scripts`](../../../test/e2e-obsidian/scripts) |
